...
/**
 * This program will convert measurements expressed in inches, feet, yards, or
 * miles into each of the possible units of measure. The measurement is input by
 * the user, followed by the unit of measure. For example: "17 feet", "1 inch",
 * "2.73 mi". Several measurements can be combined on one line of input. For
 * example: "2 miles 5 yards 1 inch". In this case, the measurements are
 * combined into one total. Abbreviations in, ft, yd, and mi are accepted.
 * Negative measurements are not allowed.
 * 
...
public class LengthConverter2 {
    ...
    /**
     * Reads past any blanks and tabs in the input. Postcondition: The next
     * character in the input is an end-of-line or a non-blank character.
     */
    ...
    /**
     * Reads the user's input measurement from one line of input. Precondition: The
     * input line is not empty. Postcondition: If the user's input is legal, the
     * measurement is converted to inches and returned. If the input is not legal,
     * the value -1 is returned. The end-of-line is NOT read by this routine.
     */
    static double readMeasurement() {
        ...
        /*
         * As long as there is more input on the line, read a measurement and add the
         * equivalent number of inches to the variable, inches. If an error is detected
         * during the loop, end the subroutine immediately by returning -1.
         */

        while (ch != '\n') {

            /*
             * Get the next measurement and the units. Before reading anything, make sure
             * that a legal value is there to read.
             */
